...
<!-- head 부분 -->
	<head>
		...
		<script>
			...
			function cu(){
				...
				<?$r_num = $_GET["num"];?>
				document.dform.action="board_modify.php?num=<?=$r_num;?>" ;
				document.dform.submit();
			}
		</script>
	</head>
	<body>
		...
		<?php
			...
			// 글 조회 수 증가 시키기
			$r_num = $_GET["num"];
			//if(preg_match("/select|union|insert|delete|information_schema/i",$r_num)) exit("no hack");
...
// 글 내용 조회 해오기
		...
		<br/>
		<p align="center">
			<form name="dform" method="post" action="board_delete_ok.php?num=<?=$r_num;?>" onsubmit="return false;">
				<font>비밀번호</font>
				<input type="password" name="b_pass" size="20">
				<input type="button" value="수정" class="btn_default btn_gray" onclick="cu();">
				&nbsp;&nbsp;
